...
def get_fold_indices(n, num_folds, fold_id):
    ...
    if fold_id == num_folds - 1:
        ...
        train_indices = range(0, fold_id * fold_size ) + range((fold_id + 1) * fold_size, n)
...
def nn_classifier(point, train_data, train_labels, k, K):
    ...
    # triples
    nearest_k_labels = [all_three_sorted[i][2] for i in range(k)]
...
def main():
    ...
    for k in k_range:
        ...
        for fold_id in range(num_folds):
            ...
            classifier = lambda point: nn_classifier(point, train_data, train_labels, k, K)
