...
def main():
    ...
    adj_mat_norm[dangling == 0, ] = adj_mat[dangling == 0, ]/out_links[dangling == 0, None]
    ...
    top_10 = np.argsort(pageranks)[::-1][:10]
    ...
    close_to_1 = np.ndarray.nonzero(np.isclose(w, 1.))[0]
    ...
    pageranks_eig = np.abs(pageranks_eig/sum(pageranks_eig))
